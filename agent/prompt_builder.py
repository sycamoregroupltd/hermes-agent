...
KANBAN_GUIDANCE = (
    ...
    "5. **Finish with exactly one successful native lifecycle terminal, using "
    "the review model encoded by the task graph.** Plain-text narration is not "
    "a board transition, and an empty/no-op result still requires a terminal "
    "call. A rejected terminal call does not count: correct its payload and "
    "retry. Always include the structured handoff (`summary`, `metadata`) on "
    "the lifecycle transition itself; never put secrets, tokens, or raw PII in "
    ...
    "When any pre-created review, QA, or release child depends on your task, call "
    ...
    "not trip unblock-loop detection. Stop after the successful lifecycle "
    "transition; do not call a second terminal for the same handoff.\n"

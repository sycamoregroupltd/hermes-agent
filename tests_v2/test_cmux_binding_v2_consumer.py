...
import datetime as dt, importlib.util, json, sqlite3, tempfile, sys
...
def setup(r):
 ...
 rec={'receipt_kind':'mac-cmux-reservation-receipt','minted_at_utc':iso(NOW-dt.timedelta(seconds=1)),'expires_at_utc':iso(NOW+dt.timedelta(seconds=300)),'canary_task':TASK,'cmux_workspace_id':WS,'cmux_surface_id':SF,'control_socket':{'cmux_daemon_version':'0.64.20','bundle_identifier':'com.cmuxterm.app'}};rec['receipt_fingerprint']=issuer.receipt_fingerprint(rec); cp=r/'c.json';put(cp,rec)

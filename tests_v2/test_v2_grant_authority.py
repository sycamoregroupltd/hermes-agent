...
def main():
    with tempfile.TemporaryDirectory() as td:
        ...
        cfg={"authority_uid":uid,"authority_gid":gid,"gate_uid":uid,"gate_gid":gid,"executor_uid":uid,"executor_gid":gid,"socket_gid":gid,"state_dir":str(fresh),"socket_path":str(root / "missing.sock"),"gate_issuer_token_file":str(root / "token"),"authority_issuer_digest_file":str(root / "digest"),"runtime_path":str(root / "runtime"),"executor_launcher":str(root / "launcher"),"executor_unit_template":str(root / "unit")}
        ...
        except ga.GrantError: missing_socket_refused=True
        finally: ga._owned_mode=original
        check("install verifier refuses incomplete authority deployment",missing_socket_refused)

...
def _contains_unsafe_gateway_action(
    ...
    *,
    ...
    # Mask inert heredoc bodies (t_7b127c8b) before the referenced-script
    # and `sh -c` payload walks below, mirroring what `_direct_lifecycle_scan`
    # (via `contains_gateway_lifecycle_command`) already does for the regex
    # pass. Without this, a quoted `python3 - <<'EOF' ... EOF` heredoc body —
    # pure Python source fed as inert stdin data to a non-shell interpreter —
    # was still shell-tokenized here: `_iter_command_segments` treats `(`/`)`
    # as shell subshell punctuation, so an ordinary `open('/tmp/kb/x.json')`
    # call inside the body got split into its own one-token segment, and
    # `_references_at`'s bare-path fallback (any standalone token containing
    # "/") misread that string literal as a directly-executed script
    # reference. When the "referenced" file was large (kanban board JSON
    # dumps routinely exceed the 1MB cap), `_read_referenced_script`'s
    # oversized-file fail-closed path (#76762) then hard-blocked the whole
    # command as a forbidden gateway-lifecycle command even though it
    # contained none — reproduced live in jarvis cron 2026-08-31 01:44
    # against a 9MB `/tmp/kb/all.json` kanban dump referenced only inside an
    # inert `python3 <<'EOF'` body. Masking here brings the referenced-script
    # and payload walks into parity with the regex pass; a heredoc that is
    # NOT provably inert (unquoted delimiter, shell consumer, unterminated
    # body) is returned unchanged by the stripper, so real lifecycle
    # commands hidden in an executable heredoc stay fully scanned.
    from tools.shell_heredoc import strip_inert_heredoc_bodies

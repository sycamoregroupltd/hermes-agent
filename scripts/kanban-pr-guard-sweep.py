...
def main():
    ...
    if all_transitioned:
        ...
        # Exit 0 on success (t_a6e61a16): this is a no_agent cron job, so
        # non-empty stdout is the deliverable (saved to the cron output dir)
        # and a non-zero exit would mark the run last_status=error even though
        # the sweep worked. "Alert on change" is carried by stdout, not by the
        # return code; exit 1 is reserved for genuine failures (exceptions).
        sys.exit(0)
    else:
        sys.exit(0)  # Silent

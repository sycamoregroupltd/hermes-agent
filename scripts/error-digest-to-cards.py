...
FIRE-ONCE-WHILE-FIX-IN-FLIGHT GUARD (t_dfa9fb08, 2026-09-10): the streak/cap machinery
above is the RIGHT shape for "don't file a blip" and "don't blow the budget" but it does
NOT stop a still-firing error from re-opening a fresh card on every stale-card
reconciliation cycle. Verified incident: `cron-jarvis-f9039bdfd04d` (overlay-drift-monitor
cron failures) re-fired 9 times in ~5.5h for the SAME root cause. Each time: (1) the PM
triages the open auto-error card, confirms the real fix (a child task) is already
running, and completes the triage card as done (correct PM behavior — it is not a
duplicate action, it is triage), (2) the underlying cron job is still failing next tick
because the fix has not landed yet, (3) the now-terminal card_id triggers the
STALE-CARD RECONCILIATION path above (t_e9849737) which clears it and retracks with a
new idempotency key, minting card #(N+1) — even though the ORIGINAL fix descendant is
still actively working the exact same root cause. This is a distinct failure mode from
the CRON-HEALTH key-drift flood (monitor-kanban-routing-hygiene skill §1 Variant B):
the idempotency KEY is stable per detection tick (no drift), but PM triage completing
the wrapper card each cycle defeats per-key dedup by making every cycle look like a
fresh open.

Guard: before creating (initial OR retrack) a card for a signature, walk the signature's
full card history (`rec["history"]`, every card_id this signature has ever been bound to,
oldest to newest — NOT just the most recent) via `task_links` on the target board's
kanban.db for an open (running/ready/blocked), non-stalled descendant (a running
descendant must have a heartbeat or start time within INFLIGHT_HEARTBEAT_MIN minutes —
see `find_inflight_descendant()`). If one exists, do NOT create a new card: append a
timestamped comment to that in-flight fix descendant instead (see `should_suppress_for_
inflight_fix()` in the "still present" loop) and record the suppression. If NO in-flight,
non-stalled descendant is found — dead worker, fix card itself blocked/crashed, or a
genuinely first-ever alert with no fix chain yet — proceed exactly as before and file a
new card. This fails OPEN toward alerting: any DB read error, missing board file, or
absence of a live descendant falls straight through to normal card creation, so the
guard can never silently swallow a real regression.
...
PROFILES_ROOT = Path(os.environ.get("ERROR_DIGEST_PROFILES", "/home/frank/.hermes/profiles"))
STATE = Path(os.environ.get("ERROR_DIGEST_STATE",
                            "/home/frank/.hermes/state/auto-error-cards.json"))
SPOOL_ARCHIVE = PROFILES_ROOT / "jarvis/state/alertmanager-spool/archive"
...
# FIRE-ONCE-WHILE-FIX-IN-FLIGHT GUARD (t_dfa9fb08): a running descendant with no
# heartbeat/activity for this long is treated as stalled, not "still fixing it" —
# falls through to normal re-create so a genuinely dead worker doesn't suppress
# alerting forever.
INFLIGHT_HEARTBEAT_MIN = int(os.environ.get("ERROR_DIGEST_INFLIGHT_HEARTBEAT_MINUTES", "180"))
OPEN_DESCENDANT_STATUSES = {"running", "ready", "blocked", "todo", "triage"}
KANBAN_BOARDS_ROOT = Path(os.environ.get("ERROR_DIGEST_KANBAN_BOARDS_ROOT",
...
def find_inflight_descendant(card_ids: list[str], board: str, now: float,
                             boards_root: Path = KANBAN_BOARDS_ROOT,
                             heartbeat_min: int = INFLIGHT_HEARTBEAT_MIN) -> str | None:
    """Walk every card this signature has EVER been bound to (oldest to newest,
    ``card_ids``) and return the id of the first non-stalled, still-open
    descendant found anywhere in its task_links subtree, else None.

    FIRE-ONCE-WHILE-FIX-IN-FLIGHT GUARD (t_dfa9fb08): a PM triaging an
    auto-error card legitimately completes it once it has confirmed the real
    fix is already in flight as a child/descendant task — that is correct PM
    behavior, not a dedup failure. But the completed wrapper card then reads as
    "terminal while the error still fires" and the stale-card reconciliation
    path (t_e9849737) mints a fresh card next tick, even though the SAME fix is
    still being worked. This walks the full history (not just the latest
    card_id) because the fix descendant may be several retrack generations
    deep — attached to card #3's chain while card #9 is the one just closed.

    A descendant counts as "in flight" only if BOTH:
      - status is in OPEN_DESCENDANT_STATUSES (running/ready/blocked/todo/triage)
      - it shows recent activity: last_heartbeat_at OR started_at OR created_at
        within heartbeat_min minutes of `now` (a running task with a dead
        worker and no heartbeat for hours is stalled, not "still fixing it" —
        must fall through to normal re-create, never silently swallow a
        regression).

    Fails OPEN (returns None -> caller proceeds with normal create) on any DB
    read error, missing board file, or no live descendant found — the guard
    must never be the reason a real, unattended regression stops alerting.
    """
    db_path = boards_root / board / "kanban.db"
    if not db_path.exists() or not card_ids:
        return None
    try:
        con = sqlite3.connect(f"file:{db_path}?mode=ro", uri=True, timeout=3.0)
        con.row_factory = sqlite3.Row
        # BFS the task_links subtree from every historical card_id.
        seen: set[str] = set()
        frontier = [c for c in card_ids if c]
        while frontier:
            ...
            frontier = [r["child_id"] for r in rows if r["child_id"] not in seen]
            for child in frontier:
                seen.add(child)
        candidates = seen - set(card_ids)  # descendants only, not the wrapper cards
        if not candidates:
            con.close()
            return None
        ...
        return None  # fail open: treat as "no in-flight descendant found"
    for row in rows:
        status = (row["status"] or "").lower()
        if status not in OPEN_DESCENDANT_STATUSES:
            continue
        activity = row["last_heartbeat_at"] or row["started_at"] or row["created_at"] or 0
        age_min = (now - activity) / 60.0
        if age_min <= heartbeat_min:
            return row["id"]
    return None


EXPECTED_GUARD_PREFIX = "Script exited with code 1\nstderr:\n"
EXPECTED_GUARD_CARD_RE = re.compile(r"t_[0-9a-f]{8}\Z")


def expected_guard_bundle_failure(job_name: str, error: str | None) -> bool:
    """Identify only the guard receipt that is expected after truthful exit 1."""
    if not job_name.startswith("guard-bundle-tick-") or not isinstance(error, str):
        return False
    if not error.startswith(EXPECTED_GUARD_PREFIX):
        return False
    try:
        receipt = json.loads(error[len(EXPECTED_GUARD_PREFIX):])
    except (TypeError, ValueError):
        return False
    return (
        isinstance(receipt, dict)
        and set(receipt) == {"status", "card", "board"}
        and receipt["status"] == "ok"
        and isinstance(receipt["card"], str)
        and EXPECTED_GUARD_CARD_RE.fullmatch(receipt["card"]) is not None
        and isinstance(receipt["board"], str)
        and bool(receipt["board"].strip())
    )


def _cron_job_names(profile_root: Path, profile: str) -> dict[str, str]:
    try:
        data = json.loads((profile_root / profile / "cron/jobs.json").read_text())
    except (OSError, ValueError, TypeError):
        return {}
    jobs = data.get("jobs", data) if isinstance(data, dict) else data
    if not isinstance(jobs, list):
        return {}
    return {str(job["id"]): str(job["name"]) for job in jobs
...
def current_errors() -> dict[str, dict]:
    ...
    # 2. Recent cron failures. Guard receipts are expected transport, not a
    #    detector failure, but every failed row must carry that exact receipt.
    _seen_exec = set()
    for _store in glob.glob(str(PROFILES_ROOT / "*" / "cron" / "executions.db")):
        real_store = os.path.realpath(_store)
        if real_store in _seen_exec:
            continue
        _seen_exec.add(real_store)
        store = _store
        profile = Path(store).parents[1].name
        try:
            ...
            rows = con.execute(
                f"select job_id, status, count(*), {error_expr} from executions"
                " where started_at >= datetime('now','-6 hours') group by 1,2,4"
            ).fetchall()
            con.close()
        except Exception:
            continue
        names = _cron_job_names(PROFILES_ROOT, profile)
        agg: dict[str, dict[str, object]] = {}
        for job, status, n, error in rows:
            item = agg.setdefault(str(job), {"counts": {}, "errors": []})
            counts = item["counts"]
            assert isinstance(counts, dict)
            counts[str(status)] = counts.get(str(status), 0) + n
            if str(status).lower() not in {"completed", "success", "ok"}:
                errors = item["errors"]
                assert isinstance(errors, list)
                errors.extend([error] * n)
        for job, item in agg.items():
            counts = item["counts"]
            errors = item["errors"]
            assert isinstance(counts, dict) and isinstance(errors, list)
            ...
            job_name = names.get(job, job)
            if bad >= 3 and good == 0 and not (
                errors and all(expected_guard_bundle_failure(job_name, error) for error in errors)
            ):
                out[f"cron-{sig(profile)}-{sig(job)}"] = {
                    "kind": "cron-failure", "board": DEFAULT_BOARD,
                    "title": f"cron job '{job_name}' failing on profile '{profile}'",
                    "detail": f"{bad} failed runs, 0 successes in the last 6h ({counts}); errors={errors}",
                }
    return out
...
def self_test() -> int:
    ...
    # --- FIRE-ONCE-WHILE-FIX-IN-FLIGHT GUARD (t_dfa9fb08, 2026-09-10) -----------
    # find_inflight_descendant() walks task_links from a signature's full card
    # history and must find a live, non-stalled fix descendant regardless of how
    # many retrack generations deep it sits, while failing open (None) whenever
    # there is genuinely nothing still working the problem.
    import tempfile as _tempfile
    with _tempfile.TemporaryDirectory() as _tmp:
        boards_root = Path(_tmp)
        board_dir = boards_root / "jarvis-os"
        board_dir.mkdir(parents=True)
        db_path = board_dir / "kanban.db"
        tcon = sqlite3.connect(str(db_path))
        tcon.execute("CREATE TABLE tasks (id TEXT PRIMARY KEY, status TEXT, "
                     "last_heartbeat_at INTEGER, started_at INTEGER, created_at INTEGER)")
        tcon.execute("CREATE TABLE task_links (parent_id TEXT, child_id TEXT, "
                     "PRIMARY KEY (parent_id, child_id))")
        now_t = 2_000_000_000.0
        # Chain: wrapper1 (terminal, gen 0) -> fix_task (running, fresh heartbeat).
        # wrapper2 (terminal, gen 1, the card that JUST closed) has no direct link
        # of its own — the fix lives under the FIRST wrapper's chain, so the walk
        # must use the full history, not just the latest card_id.
        tcon.executemany(
            "INSERT INTO tasks VALUES (?,?,?,?,?)",
            [
                ("t_wrapper1", "done", None, now_t - 5 * 3600, now_t - 5 * 3600),
                ("t_wrapper2", "done", None, now_t - 60, now_t - 60),
                ("t_fixlive", "running", now_t - 10 * 60, now_t - 4 * 3600, now_t - 4 * 3600),
                ("t_fixstalled", "running", now_t - 10 * 3600, now_t - 10 * 3600, now_t - 10 * 3600),
                ("t_fixdone", "done", None, now_t - 4 * 3600, now_t - 4 * 3600),
            ],
        )
        tcon.execute("INSERT INTO task_links VALUES ('t_wrapper1','t_fixlive')")
        tcon.commit()
        tcon.close()

        # T17: history includes both wrappers; fix lives two hops under the FIRST
        #     one -> must be found even though the just-closed card (t_wrapper2)
        #     has no direct child of its own.
        found = find_inflight_descendant(["t_wrapper1", "t_wrapper2"], "jarvis-os", now_t,
                                         boards_root=boards_root)
        assert found == "t_fixlive", f"T17: expected t_fixlive, got {found}"

        # T18: swap the live descendant for a stalled one (heartbeat 10h old, past
        #     the 180m default) -> must fail open (None), never trust a dead worker.
        tcon = sqlite3.connect(str(db_path))
        tcon.execute("DELETE FROM task_links")
        tcon.execute("INSERT INTO task_links VALUES ('t_wrapper1','t_fixstalled')")
        tcon.commit(); tcon.close()
        found = find_inflight_descendant(["t_wrapper1", "t_wrapper2"], "jarvis-os", now_t,
                                         boards_root=boards_root)
        assert found is None, f"T18: stalled descendant must not count, got {found}"

        # T19: descendant exists but is itself terminal (done) -> None.
        tcon = sqlite3.connect(str(db_path))
        tcon.execute("DELETE FROM task_links")
        tcon.execute("INSERT INTO task_links VALUES ('t_wrapper1','t_fixdone')")
        tcon.commit(); tcon.close()
        found = find_inflight_descendant(["t_wrapper1", "t_wrapper2"], "jarvis-os", now_t,
                                         boards_root=boards_root)
        assert found is None, f"T19: terminal descendant must not count, got {found}"

        # T20: no history at all (first-ever alert) -> None, fails open to create.
        found = find_inflight_descendant([], "jarvis-os", now_t, boards_root=boards_root)
        assert found is None, "T20: empty history must fail open"

        # T21: missing board db entirely -> None, fails open (never crashes).
        found = find_inflight_descendant(["t_wrapper1"], "no-such-board", now_t,
                                         boards_root=boards_root)
        assert found is None, "T21: missing board db must fail open"
    return 0


def process_active_signature(signature: str, info: dict, state: dict, now: float,
                             per_board: dict, dry_run: bool,
                             opened: list, held: list, capped: list,
                             stale_cleared: list, suppressed_inflight: list,
                             hermes_fn=None, card_status_fn=None,
                             find_inflight_fn=None,
                             boards_root: Path = KANBAN_BOARDS_ROOT) -> None:
    """Handle one currently-firing error signature: bump its streak, reconcile a
    stale card_id, apply the fire-once-while-fix-in-flight guard, and open a new
    card once MIN_STREAK/cap allow it. Mutates `state[signature]` and appends to
    the passed result lists in place — this is the exact per-signature body used
    by `main()`'s "still present" loop, factored out so the incident regression
    fixture (`fixture_suppresses_duplicate_re_fires`) can drive it directly
    against fakes instead of a live hermes/kanban board.

    hermes_fn/card_status_fn/find_inflight_fn default to the real module-level
    `hermes`/`card_status`/`find_inflight_descendant` when not injected — tests
    substitute fakes to run fully offline.
    """
    hermes_fn = hermes_fn or hermes
    card_status_fn = card_status_fn or card_status
    find_inflight_fn = find_inflight_fn or find_inflight_descendant
    ...
    if tracked:
        st, known_missing = card_status_fn(tracked, info.get("board", DEFAULT_BOARD))
        if known_missing or st in TERMINAL_STATUSES:
            ...
            # FIRE-ONCE-WHILE-FIX-IN-FLIGHT GUARD (t_dfa9fb08): remember every
            # card_id this signature has ever been bound to, so the in-flight
            # descendant search below can find a fix chain rooted several
            # retrack generations back, not just the wrapper card that just
            # went terminal.
            history = rec.setdefault("history", [])
            if tracked not in history:
    ...
    # FIRE-ONCE-WHILE-FIX-IN-FLIGHT GUARD (t_dfa9fb08), continued: this check
    # must run on EVERY tick where card_id is currently unset but a stale-card
    # history exists — not only on the single tick where the wrapper card just
    # went terminal. Once card_id is cleared it STAYS None on every following
    # tick (nothing re-tracks it until a fresh create), so if the guard only
    # fired inline with the stale-clear above, tick #2 after suppression would
    # see card_id=None + streak>=MIN_STREAK and fall straight into the normal
    # create path — recreating the exact duplicate-card storm this guard
    # exists to stop. Checking it here, unconditionally, on every card-less
    # tick makes the suppression durable for as long as the fix descendant
    # stays alive, not just for one tick.
    history = rec.get("history") or []
    if not rec.get("card_id") and history:
        _tgt = info.get("board", DEFAULT_BOARD)
        descendant = find_inflight_fn(history, _tgt, now, boards_root=boards_root) \
            if find_inflight_fn is find_inflight_descendant else find_inflight_fn(history, _tgt, now)
        if descendant:
            suppressed_inflight.append(
                f"{signature}: re-open suppressed — fix in flight at {descendant} "
                f"(streak {rec['streak']})")
            if not dry_run:
                note = (
                    f"[auto-error suppressed re-open] '{info['title']}' is still "
                    f"firing (streak {rec['streak']} consecutive checks) after its "
                    f"triage wrapper card {history[-1]} was closed. Not "
                    "opening a new auto-error card because this fix is still "
                    "in-flight for the same root cause — if this task has stalled, "
                    "the detector will mint a fresh card once it goes stale/inactive."
                )
                hermes_fn("kanban", "--board", _tgt, "comment",
                         "--author", "error-digest-to-cards", descendant, note)
            return
    ...
    rc, out = hermes_fn("kanban", "--board", board, "create",
...
def fixture_suppresses_duplicate_re_fires() -> int:
    """END-TO-END INCIDENT REGRESSION FIXTURE (t_dfa9fb08, ask item #4).

    Replays the exact overlay-drift-monitor sequence, fully offline:
      1. Detection window 1: error first seen -> held (streak < MIN_STREAK).
      2/3. Windows 2/3: streak reaches MIN_STREAK on window 3 -> card A created.
      4. PM triages card A, confirms the fix chain (a running descendant B) is
         already in flight, and completes card A (status -> done). This is
         legitimate PM behavior, not a bug.
      5/6/7. THREE more detection windows while the cron job is still failing
         and descendant B is still running: each must find card A terminal,
         walk to descendant B via find_inflight_descendant, and APPEND A
         COMMENT to B instead of creating card #2/#3/#4. Asserts exactly ONE
         comment per window (3 total) and ZERO new `create` calls after the
         first — the 2026-09-10 incident (9 duplicate cards in 5.5h) must not
         reproduce.

    Returns 0 on pass, raises AssertionError on any regression.
    """
    import tempfile as _tempfile

    calls = {"create": [], "comment": []}

    def fake_hermes(*args, **_kw):
        if "create" in args:
            n = len(calls["create"]) + 1
            # Must match the real ``t_[0-9a-f]{8}`` shape — the main() regex
            # that extracts a card id from `hermes kanban create` output only
            # matches hex, so a non-hex fake id would silently make `opened`
            # empty and falsely fail this fixture.
            card_id = f"t_fac{n:05x}"
            calls["create"].append({"args": args, "card_id": card_id})
            return 0, f"created {card_id}"
        if "comment" in args:
            calls["comment"].append(args)
            return 0, "ok"
        return 0, ""

    # card A is "done" from window 4 onward (PM closed it after confirming the
    # fix is in flight); descendant B is a live sqlite row the walk must find.
    card_a_done = {"is_done": False}

    def fake_card_status(card_id, _board):
        if card_id == "t_fac00001" and card_a_done["is_done"]:
            return "done", False
    ...
    with _tempfile.TemporaryDirectory() as _tmp:
        ...
        tcon.execute("CREATE TABLE task_links (parent_id TEXT, child_id TEXT, "
                     "PRIMARY KEY (parent_id, child_id))")
        now0 = 2_100_000_000.0
        tcon.execute("INSERT INTO tasks VALUES ('t_descendant_fix','running',?,?,?)",
                    (now0, now0, now0))
        tcon.execute("INSERT INTO task_links VALUES ('t_fac00001','t_descendant_fix')")
        tcon.commit(); tcon.close()

        state: dict = {}
        signature = "cron-jarvis-overlaydriftmonitor"
        info = {"kind": "cron-failure", "board": "jarvis-os",
               "title": "cron job 'overlay-drift-monitor' failing on profile 'jarvis'",
               "detail": "3 failed runs, 0 successes"}

        def tick(t_offset_min: float):
            per_board: dict = {}
            opened, held, capped, stale_cleared, suppressed = [], [], [], [], []
            process_active_signature(
                signature, info, state, now0 + t_offset_min * 60, per_board, False,
                opened, held, capped, stale_cleared, suppressed,
                hermes_fn=fake_hermes, card_status_fn=fake_card_status,
                find_inflight_fn=lambda hist, tgt, now, boards_root=boards_root:
                    find_inflight_descendant(hist, tgt, now, boards_root=boards_root),
            )
            return opened, held, stale_cleared, suppressed

        # Windows 1-2: below MIN_STREAK (3) -> held, no card.
        o1, h1, _, _ = tick(0)
        assert not o1 and h1, "window 1 must be held (streak 1 < MIN_STREAK)"
        o2, h2, _, _ = tick(1)
        assert not o2 and h2, "window 2 must be held (streak 2 < MIN_STREAK)"

        # Window 3: streak hits MIN_STREAK -> card A created.
        o3, _, _, _ = tick(2)
        assert len(calls["create"]) == 1, "window 3 must create exactly card A"
        assert state[signature]["card_id"] == "t_fac00001"

        # PM triages: confirms fix in flight, completes card A.
        card_a_done["is_done"] = True

        # Windows 4, 5, 6: card A now reads 'done' while the error still fires.
        # Each must find t_descendant_fix in flight and comment, NEVER create #2.
        # Only window 4 (the first tick after the wrapper card goes terminal)
        # produces a fresh `stale_cleared` entry — windows 5/6 see card_id
        # already None from window 4, so there is nothing new to "reconcile"
        # each tick; the durable signal across all three windows is
        # `suppressed_inflight`, which must fire every single time.
        for i, minute in enumerate((3, 4, 5), start=1):
            _, _, stale_cleared, suppressed = tick(minute)
            if i == 1:
                assert stale_cleared, f"window {i+3}: must detect card A as stale/terminal"
            assert suppressed, f"window {i+3}: must record a suppression, not a new card"

        assert len(calls["create"]) == 1, \
            f"REGRESSION: expected exactly 1 create total, got {len(calls['create'])} " \
            f"(the 2026-09-10 incident minted 9)"
        assert len(calls["comment"]) == 3, \
            f"expected exactly 3 comment-appends (one per suppressed window), " \
            f"got {len(calls['comment'])}"
        # Every comment must land on the actual fix descendant, not a wrapper card.
        for c in calls["comment"]:
            assert "t_descendant_fix" in c, f"comment must target the fix descendant: {c}"
    return 0
...
def main() -> int:
    ...
    if args.self_test:
        ...
        if rc != 0:
            return rc
        fixture_suppresses_duplicate_re_fires()
        print("self-test OK (including fire-once-while-fix-in-flight incident fixture)",
             file=sys.stderr)
        return 0
    ...
    now = time.time()
    opened, closed, capped, held, held_close, stale_cleared, cleaned, suppressed_inflight = \
        [], [], [], [], [], [], [], []
    ...
    for signature, info in errors.items():
        process_active_signature(
            signature, info, state, now, per_board, args.dry_run,
            opened, held, capped, stale_cleared, suppressed_inflight,
        )
    ...
    if suppressed_inflight:
        lines += ["🔇 re-open suppressed (fix already in flight for same root cause):"] + \
                 [f"   {x}" for x in suppressed_inflight]
    if cleaned:
    ...
    print(json.dumps({"status": "ok", "errors_seen": len(errors), "opened": len(opened),
                      ...
                      "suppressed_inflight": suppressed_inflight,

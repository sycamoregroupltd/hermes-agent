...
- Tracked: `scripts/*.py|*.sh|*.md` (258 files), depth-1 `profiles/*/scripts/*.py|*.sh`
  (the cron scripts that actually run — task t_4b7afeac; the keeper now carries their later
  edits too, t_881f0f42), and the previously-tracked
...
- `profiles/*/scripts/*.py|*.sh` (depth 1 only), `profiles/*/scripts/tests/*.py`, and the
  `profiles/*/scripts/frank-gate-probe/` subtree — the profile-local cron scripts that
  actually execute, mirroring the `.gitignore` carve-out exactly (t_881f0f42, 2026-09-23;
  previously only the two keeper wrappers below were permitted)
...
**Known allowlist gap — CLOSED (2026-09-23, t_881f0f42).** Until this date `.gitignore`
re-included *every* depth-1 `profiles/*/scripts/*.py|*.sh` (task t_4b7afeac) while the
keeper's `is_allowed()` permitted only `profiles/devops/scripts/automation-vc-keeper.sh`,
so the keeper carried no later edits to any other profile script: `profiles/jarvis/scripts/`
was the ground truth for every guard-bundle check, yet none of it reached
`fleet/automation-vc`. `is_allowed()` now permits exactly the `.gitignore` shape
(depth-1 `*.py|*.sh` + `scripts/tests/*.py` + the `frank-gate-probe` subtree). Because
`discover()` is tracked-first by default, the widening can only carry *later edits to
already-tracked* profile scripts; a brand-new/untracked profile script still needs an
explicit `--include-untracked` review.

**Tick-abort bug fixed in the same change (t_881f0f42).** A candidate that is tracked in
the live repo but deleted from the working tree (`git ls-files` still lists it) and absent
from the branch has no blob in the temp worktree, so `git add -- <path>` died with
`fatal: pathspec ... did not match any files` and aborted the whole tick before every
other file was committed. Staging now skips worktree-absent paths and names them on
stderr (5 at the time of the fix: `scripts/deploy_sycodeserver.sh`,
`scripts/pit-context-join.sh`, `scripts/run-datalake-export.sh`,
`scripts/test_dgx_board_sweep_staleness.py`, `scripts/test_report_to_board.py`).
...
4. Stage only that pathset. **Deletions are never staged automatically** — removing a file
   from the branch is always a deliberate human commit. Paths absent from the worktree
   (tracked live, deleted on disk, not on the branch) are skipped and reported on stderr
   instead of aborting the tick (t_881f0f42).

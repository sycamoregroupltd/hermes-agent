...
def is_phantom(
    assignee: str | None,
    profiles: set[str],
    external_seats: frozenset[str] | set[str] = frozenset(),
) -> bool:
    if assignee is None:
...
def scan_board(
    ...
    board: str,
    profiles: set[str],
    external_seats: frozenset[str] | set[str],
) -> list[dict]:
...
def probe(
    ...
    external_seats: frozenset[str] | set[str] | None = None,
) -> list[dict]:
...
def selftest() -> int:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        hits = probe(
            root / "kanban" / "boards",
            root / "profiles",
            frozenset({"external-claude-x", "fable"}),
        )

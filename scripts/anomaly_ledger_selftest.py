...
print("== Flow B: subsequent run dedupes (comment, no new ticket) ==")
r2 = L.record_anomaly(
    ledger, report_class="Fusion Calibration", source="calibration_001.log",
...
check(len(h.comments) == 1, "one comment appended on existing ticket")
check(h.comments[0][0] == tid, "comment posted to the original ticket id")
check(r2["occurrences"] == 2, "occurrence counter incremented to 2")
check("occurrence #2" in h.comments[0][1], "comment body marks occurrence #2")
...
check(len(h.comments) == 2, "one more comment (RESOLVED)")
check(h.comments[1][1].startswith(L.RESOLVED_COMMENT_PREFIX), "RESOLVED comment text correct")
check(ts(3) in h.comments[1][1], "RESOLVED comment carries timestamp")
...
check(r5["action"] == "resolved", "still reports resolved (entry cleared)")
check(len(h3.completed) == 0, "did NOT auto-complete an in_progress task")
check(any("already in 'in_progress'" in c[1] for c in h3.comments), "notes it left the task open")
...
check(L.load_ledger(bad) == {"version": 1, "entries": {}}, "corrupt ledger -> empty, no crash")

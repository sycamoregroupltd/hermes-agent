...
def main():
    ...
    write_markdown_atomic(
        report_path,
        report,
        title=f"Watchlist: SHORT 1h LOW vol + NEUTRAL macro — {today_str}",
        type="task-evidence",
        status="active",
        created=today_str,
        updated=today_str,
        confidence="medium",
        tags=["sycode", "watchlist", "short", "one-hour", "low-volatility", "neutral-macro"],
        sources=["sycodetrading-supabase-db:signal_journeys"],
        project="sycode-trading",
        owners=["trading-devops"],
        knowledge_tier="evidence",
        generated=True,
        generator="short-1h-low-vol-neutral-watchlist.py",

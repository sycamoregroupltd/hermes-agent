...
# Terminal-by-approve corpus (t_6334ff49 / t_a7d76178): the A3 packet requires
# fixtures (a) approve->done stays terminal, (b) done->blocked WITHOUT re-open
# rejected/flagged, (c) done->blocked WITH reviewer re-open allowed. Assert each
# required fixture is present with the exact expected terminal verdict so the
# corpus cannot silently lose coverage for the terminal rule.
_TERMINAL_REQUIRED = [
    # (name-substring, expected_verdict)
    ("terminal-a-approve-done-stays-terminal", "terminal"),
    ("terminal-b-done-blocked-no-reopen-rejected", "regress_blocked"),
    ("terminal-c-done-blocked-with-reopen-allowed", "regress_allowed"),
]
for _needle, _verdict in _TERMINAL_REQUIRED:
    _hit = any(
        _needle in fixture.get("name", "")
        and (fixture.get("terminal") or {}).get("expected_verdict") == _verdict
        for fixture in fixtures
    )
    if not _hit:
        raise SystemExit(
            f"maintenance contract missing terminal-by-approve corpus fixture {_needle} (expected verdict {_verdict})"
        )

print("PASS maintenance-contract: paired negatives, changed_files-aware guardrails, and terminal-by-approve corpus a/b/c present")
PY
...
run_terminal_corpus_case() {
  # Terminal-by-approve lock-gate (t_6334ff49 / t_a7d76178): assert the exact
  # classifier verdict for a fixture that carries a `terminal` sub-object.
  local name="$1" terminal_json="$2"
  local status approval reopen expected_verdict
  status=$(python3 -c "import json,sys;print(json.loads(sys.argv[1])['status'])" "$terminal_json" 2>/dev/null)
  approval=$(python3 -c "import json,sys;print('1' if json.loads(sys.argv[1]).get('approval') else '0')" "$terminal_json" 2>/dev/null)
  reopen=$(python3 -c "import json,sys;print('1' if json.loads(sys.argv[1]).get('reopen') else '0')" "$terminal_json" 2>/dev/null)
  expected_verdict=$(python3 -c "import json,sys;print(json.loads(sys.argv[1])['expected_verdict'])" "$terminal_json" 2>/dev/null)
  local args=()
  [ "$approval" = "1" ] && args+=(--approval)
  [ "$reopen" = "1" ] && args+=(--reopen)
  local verdict
  verdict=$(python3 "$classifier" --terminal --status "$status" "${args[@]}" 2>/dev/null) || verdict="unsupported"
  if [ "$verdict" != "$expected_verdict" ]; then
    printf 'FAIL %s: expected terminal verdict %s got %s\n' "$name" "$expected_verdict" "$verdict" >&2
    exit 1
  fi
...
while IFS=$'\x1f' read -r name tid expected summary metadata_json expected_class terminal_json; do
  ...
  if [ -n "${terminal_json:-}" ]; then
    run_terminal_corpus_case "fixture-corpus:${name}" "$terminal_json"
  fi
...
for fixture in json.load(open(sys.argv[1], encoding="utf-8")):
    print("\x1f".join([
        ...
        json.dumps(fixture.get("terminal", None), sort_keys=True) if fixture.get("terminal") else "",
    ]))
PY
)

...
# task is simply re-dispatched on the next tick (its claim is released as
# normal), and the stranding is gone.
...
_SPAWN_TRANSIENT_ERROR_SUBSTRINGS = (
    "[errno 11]", "[errno 11]", "resource temporarily unavailable",
    ...
    *,
    ...
    with:
      ...
      * **bounded exponential-backoff retry** on transient clone failures
        ...
        task's ``consecutive_failures`` is NOT incremented and it is simply
        re-dispatched on the next tick instead of being stranded in
        ``blocked`` with a stale error.
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            # the task — it is just re-dispatched on the next tick).

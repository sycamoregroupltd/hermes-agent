...
def promote_task(
    ...
    *,
    ...
    if cur_status == "blocked":
        # A blocked card is not cleared by arbitrary approval-shaped prose.
        # In particular, an implementer/self comment must not manufacture the
        # ``promoted_manual`` event that _has_unresolved_block treats as an
        # unblock.  Keep this provenance gate at the status transition (rather
        # than relying on dispatch-time interpretation), so a later redispatch
        # cannot erase the control-plane evidence.
        identity = conn.execute(
            "SELECT assignee FROM tasks WHERE id = ?", (task_id,)
        ).fetchone()
        assignee = identity["assignee"] if identity else None
        if (
            isinstance(actor, str)
            and isinstance(assignee, str)
            and _canonical_assignee(actor) == _canonical_assignee(assignee)
        ):
            return False, "implementer cannot promote its own blocked task"

        # A parent with an active review child remains gated until that child is
        # terminal. This closes the t_822815b9 replay shape: the implementer
        # cannot auto-clear the parent while its independent review gate is
        # still pending. ``force`` deliberately does not bypass this invariant.
             ...
             WHERE l.parent_id = ?
               ...
               AND EXISTS (
                   SELECT 1 FROM task_events e
                    WHERE e.task_id = c.id AND e.kind = 'review_requested'
               )
             LIMIT 1
            """,
            (task_id,),
        ...
        if review_child is not None:
            return False, (
                "blocked parent has an active review-child gate; "
                "complete or archive the review child before promotion"
            )

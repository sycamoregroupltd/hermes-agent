...
def _resolve_respawn_guard_event_suppression_seconds() -> int:
    ...
    ``DEFAULT_RESPAWN_GUARD_EVENT_SUPPRESSION_SECONDS`` when absent, empty,
    non-integer, or negative. A value of 0 disables age-based suppression
    (identical reasons re-emit every tick — restores the legacy noisy
    per-tick behaviour for debugging). Mirrors
...
def _dispatch_once_locked(
    ...
    *,
    ...
    for row in review_rows:
        ...
        if guard_reason is not None:
            result.respawn_guarded.append((row["id"], guard_reason))
            # Same bounded throttle as the ready lane above: the in-memory
            # telemetry fires every tick, but the durable event row is written
            # at most once per (task, reason) until the reason changes or the
            # suppression window elapses. Decision logic is untouched.
            if not dry_run and _should_emit_respawn_guarded_event(
                conn, row["id"], guard_reason
            ):
                with write_txn(conn):

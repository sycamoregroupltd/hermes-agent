...
DEFAULT_URL = ""  # SECURITY (chip task_5c4d5f67, 2026-08-05): no hardcoded
# credential here — the DSN (password included) used to be baked in and leaked
# into pushed git history. resolve_db() derives it from ENV_FILE at runtime and
# raises if it cannot, rather than falling back to a literal password.
...
def resolve_db():
    ...
    if not m:
        raise RuntimeError(
            f"cannot derive a usable DATABASE_URL from {ENV_FILE} "
            "(no hardcoded fallback by design — chip task_5c4d5f67)")
...
def psql(db, sql):
    ...
    # statement_timeout=0: the union of max()-aggregates seq-scans hot tables and
    # the DB default timeout kills it ("canceling statement due to statement
    # timeout", proven 2026-08-05) — same fix as sycode_candle_per_symbol_freshness.py.
    # subprocess timeout=50 below still bounds the wall clock.

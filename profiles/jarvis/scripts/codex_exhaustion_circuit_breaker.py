...
# Quiet once-per-day marker when DISPATCH_JOB is absent from live jarvis cron
# (removed fleet-dispatch-loop; id may linger only in cron-snapshots).
MISSING_DISPATCH_STATE = os.environ.get(
    "CODEX_BREAKER_MISSING_DISPATCH_STATE",
    "/home/frank/.hermes/state/codex-circuit-breaker-missing-dispatch-job.json",
)
# STILL-SELECTIVE used to append every ~5m tick while Codex exhausted — 16k+ lines by 2026-09-05.
# Keep first SELECTIVE-TRIPPED + heartbeat at most once per hour for the same episode.
STILL_SELECTIVE_THROTTLE_SECS = int(os.environ.get("CODEX_STILL_SELECTIVE_THROTTLE_SECS", str(60 * 60)))
STILL_SELECTIVE_HB = os.environ.get(
    "CODEX_STILL_SELECTIVE_HB",
    "/home/frank/.hermes/state/codex-circuit-breaker-still-selective-hb.json",
)
...
def _combined_output(r: subprocess.CompletedProcess[str]) -> str:
    return f"{(r.stdout or '').strip()}\n{(r.stderr or '').strip()}".strip()


def _is_job_missing_output(text: str) -> bool:
    """True when hermes pause/resume reports the DISPATCH_JOB id/name is gone."""
    t = (text or "").lower()
    if not t:
        return False
    if "not found" not in t:
        return False
    return "job" in t or DISPATCH_JOB.lower() in t


def _read_missing_dispatch_state() -> dict[str, Any]:
    try:
        return load_json(MISSING_DISPATCH_STATE)
    except Exception:
        return {}


def _already_noop_logged_today() -> bool:
    st = _read_missing_dispatch_state()
    return bool(st.get("missing") and st.get("day") == _utc_day() and st.get("job_id") == DISPATCH_JOB)


def _record_missing_dispatch_noop(action: str, reason: str, detail: str) -> None:
    """Missing DISPATCH_JOB is expected post-removal — quiet no-op, log <=1/day."""
    day = _utc_day()
    should_log = not _already_noop_logged_today()
    atomic_write_json(
        MISSING_DISPATCH_STATE,
        {
            ...
            # Do NOT recreate fleet-dispatch-loop; selective mode uses allowlist only.
        },
        mode=0o644,
    )
    if should_log:
        log(
            f"DISPATCH-{action}-NOOP job-missing once-per-day "
            f"job={DISPATCH_JOB} reason={reason} detail={(detail or '')[:160]}"
        )


def _clear_missing_dispatch_state() -> None:
    st = _read_missing_dispatch_state()
...
def _should_log_still_selective(episode_id: str) -> bool:
    """Rate-limit STILL-SELECTIVE heartbeats to once per STILL_SELECTIVE_THROTTLE_SECS per episode."""
    try:
        st = load_json(STILL_SELECTIVE_HB)
    except Exception:
        st = {}
    now = time.time()
    last_at = float(st.get("at") or 0)
    last_ep = st.get("episode_id")
    if last_ep == episode_id and (now - last_at) < STILL_SELECTIVE_THROTTLE_SECS:
        return False
    atomic_write_json(
        STILL_SELECTIVE_HB,
        {"at": now, "episode_id": episode_id, "iso": iso_utc()},
        mode=0o644,
    )
    return True
...
def ensure_dispatch_resumed(reason: str) -> None:
    # After first daily NOOP for a removed DISPATCH_JOB, skip hermes entirely.
    if _already_noop_logged_today():
        return
    r = run([HERMES, "cron", "resume", DISPATCH_JOB])
    out = _combined_output(r)
    if _is_job_missing_output(out):
        _record_missing_dispatch_noop("RESUME", reason, out)
        return
    _clear_missing_dispatch_state()
    log(f"DISPATCH-RESUME-CHECK {reason}; resume rc={r.returncode} {(r.stdout or '').strip()[:120]}")
...
def pause_dispatch(reason: str) -> None:
    ...
    out = _combined_output(r)
    if _is_job_missing_output(out):
        _record_missing_dispatch_noop("PAUSE", reason, out)
        return
    _clear_missing_dispatch_state()
    log(f"TRIPPED ({reason}); pause rc={r.returncode} {(r.stdout or '').strip()[:120]}")
...
def main() -> None:
    ...
    if not ok:
        ...
        if first_trip or _should_log_still_selective(episode_id):

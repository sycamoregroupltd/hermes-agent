...
#        - persists         -> comment fresh output on the existing ticket
#        - green again      -> comment RESOLVED + auto-complete + recovery ping
...
# (/home/frank/.hermes/scripts) so there is a single source of truth.
...
def core_run(
    ...
    # Self-heal: any ledger entry whose key is no longer in the current scan.
    current_keys = {
        ledger._entry_key(a.report_class, a.source_file) for a in anomalies
    }
    resolved = []
    for entry in ledger.list_active(book):
        key = ledger._entry_key(entry["report_class"], entry["source"])
    ...
    if not quiet:
        ...
        print(f"created={len(created)} deduped={len(deduped)} "
              f"resolved={len(resolved)} active={summary['active_tickets']}")
    return summary

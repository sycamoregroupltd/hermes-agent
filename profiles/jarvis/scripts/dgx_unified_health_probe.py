...
def _discover_board_dbs() -> list[Path]:
    ...
    Discovery is STRUCTURAL (glob), not a hardcoded pair of boards — a board
    added by creating boards/<label>/kanban.db is picked up WITHOUT a code
    change (t_bf11a0ce binding spec addition, 2026-08-03). A hardcoded pair
    recreates the identical blind spot the moment a board is added.
    """
    if not BOARDS_DIR.exists():
        return []
...
def _ready_backlog_check_name(label: str) -> str:
    """Stable check-row name for a board's ready-backlog telemetry.

    Legacy names are preserved for jarvis-os and sycode-trading (existing
    consumers match them); other boards get <sanitized-label>_ready_backlog.
    """
    legacy = {
        "jarvis-os": "jarvis_ready_backlog",
        "sycode-trading": "sycode_trading_ready_backlog",
    }
    if label in legacy:
        return legacy[label]
    return re.sub(r"[^A-Za-z0-9]+", "_", label).strip("_") + "_ready_backlog"


def scan_all_boards_ready_backlog(
    now: dt.datetime | None = None,
) -> tuple[dict[str, dict[str, Any]], list[tuple[str, str | None, float | None]]]:
    """Per-board ready backlog for EVERY tracked board.

    Returns (by_board, warn_boards):
      by_board    — label -> record from _scan_board_ready_backlog
      warn_boards — (label, oldest_ready_task_id, age_days) triples for boards
                    whose oldest ready exceeds READY_BACKLOG_WARN_DAYS

    Observability-only: warn flags degrade PASS -> WARN, never BLOCK
    (t_7c589e9c / t_bf11a0ce contracts preserved).
    """
    now = now or utc_now()
    by_board: dict[str, dict[str, Any]] = {}
    warn_boards: list[tuple[str, str | None, float | None]] = []
    for db in _discover_board_dbs():
        label = db.parent.name
        rec = _scan_board_ready_backlog(db, label, now)
        by_board[label] = rec
        if rec.get("warn"):
            warn_boards.append(
                (label, rec.get("oldest_ready_task_id"), rec.get("oldest_ready_age_days"))
            )
    return by_board, warn_boards
...
def main() -> int:
    ...
    # Per-board ready backlog (t_bf11a0ce + binding spec addition 2026-08-03):
    # scan EVERY tracked board structurally (BOARDS_DIR/*/kanban.db), NOT a
    # hardcoded jarvis-os + sycode-trading pair. A stalled approval/review on
    # any board is visible in the fleet verdict; a board added later is picked
    # up WITHOUT a code change (a hardcoded pair recreates the blind spot the
    # moment a board is added).
    board_backlogs, warn_boards = scan_all_boards_ready_backlog(now)
    for _label in sorted(board_backlogs):
        _bl = board_backlogs[_label]
        checks.append({"name": _ready_backlog_check_name(_label), "ok": True,
                       "detail": _bl.get("detail", ""),
                       "fork_resource_pressure": False,
                       "observability_only": True,
                       "warn": _bl.get("warn", False)})

    # Back-compat record aliases: consumers read the legacy keys
    # jarvis_ready_backlog / sycode_trading_ready_backlog. Resolve from the
    # structural scan when the board exists, else fall back to the constants
    # (keeps the record shape stable even for a board dir that is absent).
    ready_backlog = board_backlogs.get("jarvis-os") or _scan_board_ready_backlog(
        JARVIS_OS_KANBAN_DB, "jarvis-os", now)
    sycode_ready_backlog = board_backlogs.get("sycode-trading") or _scan_board_ready_backlog(
    ...
    # tracked boards (t_bf11a0ce binding spec addition 2026-08-03).
    warn_boards = warn_boards  # from scan_all_boards_ready_backlog above
    ...
    if not blocked and not degraded:
        ...
        for _label in sorted(board_backlogs):
            ...
            if _bl.get("available"):
                lines.append(f"  - {_label}: {_bl.get('detail')}")

#!/usr/bin/env bash
# t_1b0f7543 spool-relay wrapper for CrossPMFanout.
# Runs the original monitor (cross-pm-fanout.sh); if it emits non-empty stdout, writes an
...
set -uo pipefail
ORIGINAL="/home/frank/.hermes/profiles/jarvis-os-pm/scripts/cross-pm-fanout.sh"
...
if [ -n "$OUT" ]; then
  python3 /home/frank/.hermes/scripts/spool_alert_write.py \
    --spool "$SPOOL_DIR" --alertname CrossPMFanout --severity warning \

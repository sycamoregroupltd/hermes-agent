...
def is_expected_guard_bundle_alert(job, error):
    """Return true for a delivered guard alert that intentionally exits 1.

    Guard bundles preserve non-zero check results so the cron remains visibly
    red, while report-to-board successfully creates the consumer card. That
    transport signature is an expected alert, not a dead job pin. Keep this
    narrowly scoped to the bundle name, exact wrapper prefix, and a valid JSON
    delivery receipt so unrelated exit-1 errors still alert.
    """
    if not isinstance(job, dict) or not (job.get("name") or "").startswith("guard-bundle-tick-"):
        return False
    ...
    if not error.startswith(prefix):
        return False
    try:
        receipt = json.loads(error[len(prefix):].strip())
    except (TypeError, ValueError):
        return False
    return (
        isinstance(receipt, dict)
        and receipt.get("status") == "ok"
        and isinstance(receipt.get("card"), str)
        and bool(receipt["card"])
        and isinstance(receipt.get("board"), str)
        and bool(receipt["board"])
    )

def iter_profile_dirs():
    """Yield (real_name, dirname) for every distinct profile store, deduped by
    realpath so a symlinked profile alias (e.g. sycode-trading -> sycode-trading-pm,
    where two seats share ONE physical cron store) is scanned exactly once, under
    its real name. Mirrors dgx_cron_health_canary.iter_profile_jobs()."""
    seen_real = set()
    for entry in sorted(os.listdir(PROFILES)):
        if entry.startswith("."):
            continue
        pdir = os.path.join(PROFILES, entry)
        real = os.path.realpath(pdir)
        if real in seen_real:
            continue
        seen_real.add(real)
        real_name = os.path.basename(real.rstrip("/"))
        yield real_name, entry

def main():
    findings = []  # list of dicts
    for profile, pdir in iter_profile_dirs():
        for j in load_jobs(pdir):
            ...
            if cls:
                findings.append({
                    "cls": cls, "profile": profile, "job": name, "id": jid,
            ...
            if enabled:
                ...
                if es:
                    ...
                    if is_expected_guard_bundle_alert(j, last_err):
                        continue
                    findings.append({
                        "cls": "ERROR-STREAK", "profile": profile, "job": name,

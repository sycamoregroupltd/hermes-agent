...
# ---------------------------------------------------------------------------
# Worker-liveness watchdog + dead-pid harness_defect blocks (t_f9b3dba7)
#
# A worker that dies mid-run with a dead pid used to sit in ``running`` until
# the next full dispatch tick (60s default) reclaimed it, and when the
# breaker finally tripped the auto-block was written with no block_kind and no
# comment — a card stuck in invisible blocked limbo. These tests pin the
# fixes: (a) a single dead-pid crash re-queues the card to ``ready`` for a
# retry; (b) repeated dead-pid crashes past the failure limit hard-block as
# typed ``harness_defect`` (routed to devops) with a visible AUTO-BLOCK comment
# that carries captured worker stderr; (c) ordinary nonzero-exit crashes keep
# blocking as ``capability`` exactly as before; (d) ``sweep_worker_liveness``
# is the fast sub-tick reclaim path and respects the dispatch tick lock.
# ---------------------------------------------------------------------------
...
def test_dead_pid_repeated_crashes_hard_block_harness_defect(kanban_home):
    ...
    ``harness_defect`` routed to devops — never ``capability``, and with a
    visible AUTO-BLOCK comment (captured stderr when a worker log exists).
    """
    import hermes_cli.kanban_db as _kb
    conn = kb.connect()
    try:
...
def test_dead_pid_auto_block_captures_worker_log_tail(kanban_home):
    """When a worker log exists, the harness_defect AUTO-BLOCK comment carries
    the captured worker stderr tail as evidence (t_f9b3dba7 acceptance: "
    terminal-block with captured stderr").
    """
    import hermes_cli.kanban_db as _kb
    conn = kb.connect()
    try:
        tid = kb.create_task(conn, title="deadpid-stderr", assignee="worker")

        # Write a worker log with a realistic crash trace before the reaper
        # pass runs, so the block comment captures it.
        log_path = _kb.worker_log_path(tid)
        log_path.parent.mkdir(parents=True, exist_ok=True)
        log_path.write_text(
            "Traceback (most recent call last):\n"
            "  File \"run_agent.py\", line 999, in <module>\n"
            "hermes: error: unknown skill(s): kanban-worker\n",
            encoding="utf-8",
        )

        # Two dead-pid passes to trip the breaker.
        _drive_dead_pid(conn, tid, 994010)
        _drive_dead_pid(conn, tid, 994011)
        task = kb.get_task(conn, tid)
        assert task.status == "blocked"
        assert task.block_kind == "harness_defect"
        ...
        auto_blocks = [b for b in bodies if b.startswith("AUTO-BLOCK")]
        assert len(auto_blocks) == 1
        assert "unknown skill(s): kanban-worker" in auto_blocks[0], (
            "harness_defect block must capture the worker stderr tail"
        )
        assert "Captured worker stderr tail" in auto_blocks[0]
    finally:
        conn.close()
...
def test_sweep_worker_liveness_reclaims_dead_pid(kanban_home):
    """``sweep_worker_liveness`` is the fast sub-tick reclaim path: it reaps
    and reclaims a dead-pid worker's card without a full dispatch tick
    (t_f9b3dba7 acceptance: detect dead pid within 30s).
    """
    import hermes_cli.kanban_db as _kb
    conn = kb.connect()
    try:
        tid = kb.create_task(conn, title="liveness", assignee="worker")
        host_prefix = _kb._claimer_id().split(":", 1)[0]
        claimed = _kb.claim_task(conn, tid, claimer=f"{host_prefix}:mock")
        assert claimed is not None
        _kb._set_worker_pid(conn, tid, 995000)

        original_alive = _kb._pid_alive
        _kb._pid_alive = lambda p: False
        try:
            crashed, auto_blocked = _kb.sweep_worker_liveness()
        finally:
            _kb._pid_alive = original_alive

        assert tid in crashed
        assert tid not in auto_blocked
        task = kb.get_task(conn, tid)
        assert task.status == "ready", (
            f"sweep must reclaim dead-pid worker to ready, got {task.status}"
        )
    finally:
        conn.close()

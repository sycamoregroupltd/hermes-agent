...
def test_blocked_implementer_cannot_self_promote_after_approval_shaped_comment(
    home: Path,
) -> None:
    """Replay the t_822815b9 failure: self-authored approval text must not
    create a clearing promotion or make the blocked parent claimable."""
    with kb.connect_closing() as conn:
        tid = kb.create_task(conn, title="blocked-parent", assignee="implementer")
        _make_blocked_via_sql(conn, tid, "review-required")
        ok, err = kb.promote_task(
            conn, tid, actor="implementer", reason="REVIEW_VERDICT=APPROVED",
            force=True,
        )
        assert not ok
        assert "implementer" in (err or "")
        assert kb.get_task(conn, tid).status == "blocked"
        assert kb._has_unresolved_block(conn, tid) is True
        assert kb.claim_task(conn, tid, claimer="dispatcher") is None


def test_blocked_parent_with_active_review_child_cannot_auto_clear(
    home: Path,
) -> None:
    """An active independent review child is an approval gate, not an
    approval event; force promotion must remain fail-closed."""
    with kb.connect_closing() as conn:
        parent = kb.create_task(conn, title="blocked-parent", assignee="implementer")
        child = kb.create_task(conn, title="independent review", assignee="reviewer")
        assert kb.request_review(conn, child, reviewer="reviewer", force=True)
        kb.link_tasks(conn, parent, child)
        _make_blocked_via_sql(conn, parent, "review-required")

        ok, err = kb.promote_task(conn, parent, actor="operator", force=True)
        assert not ok
        assert "review-child" in (err or "")
        assert kb.get_task(conn, parent).status == "blocked"
        assert kb._has_unresolved_block(conn, parent) is True

...
# ---------------------------------------------------------------------------
# Respawn-guard event throttle (t_653bae20 regression fixture)
#
# Validates the bounded state-transition + heartbeat throttle implemented in
# t_31e51e8d (design t_e3308956). The dispatcher ticks roughly every minute,
# so a ready card held by a persistent guard (e.g. ``active_pr`` while a PR
# waits in review) used to accumulate one identical ``respawn_guarded
# {reason}`` row PER TICK (~1,440 rows/day/task). The agreed rule is a
# state-transition + bounded-heartbeat throttle: emit on first entry, on
# reason change, and at most once per suppression window thereafter. The
# spawn-guard *decision* (``check_respawn_guard``) is untouched — only the
# redundant durable event rows are suppressed.
#
...
# against any correct implementation of the agreed rule. Proven RED (these
# suppression tests fail) when the emitter is forced to write every tick.
# ---------------------------------------------------------------------------
